...
import { ChevronRight, MapPin, Clock, Star, CheckCircle2, SlidersHorizontal, Globe } from 'lucide-react';

interface InternationalCountryPackagesProps {
  onOpenInquiry: (dest?: string) => void;
}

export const InternationalCountryPackages: React.FC<InternationalCountryPackagesProps> = ({ onOpenInquiry }) => {
  const { countrySlug } = useParams<{ countrySlug: string }>();
  const { internationalCountries, packages } = useData();

  const currentCountry = internationalCountries.find(c => c.slug === countrySlug) || internationalCountries[0];
  const countryPackages = packages.filter(p => p.parentSlug === currentCountry.slug);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 space-y-8">
      ...
      <div className="flex items-center gap-2 text-xs text-slate-500">
        ...
        <Link to="/international" className="hover:text-[#00AEEF]">International Packages</Link>
        <ChevronRight className="w-3.5 h-3.5" />
        <span className="text-slate-900 font-semibold">{currentCountry.name}</span>
      </div>
      ...
      <div className="relative rounded-3xl overflow-hidden bg-slate-900 text-white p-6 md:p-10 shadow-xl border border-slate-800">
        <div className="absolute inset-0 z-0 opacity-40">
          <ImageWithFallback
            src={currentCountry.bannerImage || currentCountry.image}
        ...
        <div className="relative z-10 space-y-3 max-w-3xl">
          <span className="bg-[#FDB813] text-slate-900 text-[11px] font-bold px-3 py-1 rounded-full uppercase tracking-wider inline-flex items-center gap-1">
            <Globe className="w-3.5 h-3.5" /> {currentCountry.tag}
          </span>
          <h1 className="text-3xl md:text-5xl font-extrabold">{currentCountry.name} Holidays & Tours</h1>
          ...
          <div className="pt-2 flex flex-wrap gap-2 items-center">
            <span className="text-xs text-slate-400 font-semibold">Key Cities & Regions:</span>
            {currentCountry.regions.map((reg, idx) => (
              <button
                key={idx}
      ...
      <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-2 text-slate-800 font-bold text-xs uppercase tracking-wider">
          ...
          <span>Filter {currentCountry.name} Packages</span>
        </div>

        <div className="flex flex-wrap items-center gap-4 text-xs">
          {/* Sub Region Filter */}
          <div className="flex items-center gap-2">
            <label className="text-slate-500">Region/City:</label>
            <select
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <option value="budget">Value Explorer</option>
              <option value="mid">Premium Deluxe</option>
              <option value="luxury">Ultra Luxury VIP</option>
            </select>
          </div>
      ...
      {filteredPackages.length === 0 && (
        <div className="text-center py-16 bg-white rounded-3xl border border-slate-200 p-8 space-y-3">
          ...
          <p className="text-xs text-slate-500">Try resetting filters to view all packages in {currentCountry.name}.</p>
          <button
            onClick={() => {
              setSelectedRegion('all');
              setMaxBudget(300000);
